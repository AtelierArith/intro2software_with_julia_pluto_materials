...
# ╔═╡ ea57eaa4-ccb5-11ee-3e7a-63d2e6a789bf
begin
	using PythonCall
	using CondaPkg
	using Plots
...
# ╔═╡ 8fb849af-5578-45d6-a809-07d7772b2003
scipy = pyimport("scipy")

# ╔═╡ 06ab026a-ca8f-4c33-8fbe-efcf0086a36f
function f(t, u)
	return @. u * (one(u) - u)
end

# ╔═╡ 05522ea3-e8a2-47e3-b846-dd79ad23f6e7
begin
	u_0 = [1e-3]
	tspan = (0, 20.0)
	sol = scipy.integrate.solve_ivp(f, tspan, u_0, dense_output=true)
	sol.message
end

# ╔═╡ 33cdccfa-4cd7-4646-b23b-34c277812873
begin
	t = range(0, 20.0, length=101)
	logistic(t, u_0 = 1e-3) = u_0 / (u_0 + (one(u_0) - u_0) * exp(-t))

	plot(title="du/dt = f(u(t))")
	plot!(t, logistic.(t), lw=3)
	
	scatter!(
		t, PyVector(sol.sol(t).flatten()),
		label="dense output"
	)
	scatter!(
		PyVector(sol.t.flatten()), PyVector(sol.y.flatten()), 
...
manifest_format = "2.0"
project_hash = "d2a7b76b06cafdbc04ab49e3bc2d85bca739e87a"
